...
{
    public class PeopleRepositoryImplementation : GenericRepositoryImplementaton<PeopleEntity>, IPeopleRepository
    {
        ...
        public PeopleRepositoryImplementation(IWebHostEnvironment webHostEnvironment,
        ...
        public async Task<Guid> AddPeople(PeopleModel userModel)
        {
        ...
        public async Task<string> UpdatePeople(Guid Id, PeopleModel userModel)
        {
            ...
            if (userModel.ImageFile != null && userModel.ImageFile.Length > 0)
            {
                ...
                using (var FileStream = new FileStream(FilePath, FileMode.Create))
                {
                    userModel.ImageFile.CopyToAsync(FileStream).ConfigureAwait(false);
                }
        ...
        public async Task<bool> IsUniqueUserName(string UserName, Guid UserId)
        {
            var Reserved = await Table().AnyAsync(x => x.UserName == UserName && !x.IsDeleted && x.Id != UserId);

            if (Reserved)
            {
                throw new CustomException("شناسه کاربری متعلق به شخص دیگری است");
            }
        ...
        public async Task<List<PeopleModel>> GetAllPeople(string? FirstFilterOn = null, string? FirstFilterQuery = null,
            ...
            bool ShowDeletedOnes = false, int pageNumber = 1, int PageSize = 100)
        {
            ...
            var UsersMappedList = new List<PeopleModel>();

            foreach (var entity in UsersList)
            {
                var MapResult = new PeopleModel
                {
        ...
        public async Task<List<PeopleModel>> GetAllPaginatedPeople(int PageNumber = 1, int PageSize = 10)
        {
            var CachedKey = $"PaginatedUsers_{PageNumber}_{PageSize}";

            CachedKeyLists.Add(CachedKey);

            var CachedUsers = await _cacheManagerService.GetAsync<List<PeopleModel>>(CachedKey, async() =>
            {
        ...
        public async Task<PeopleModel> GetPeopleById(Guid Id, bool ShowIfIsDeleted = false)
        {
            ...
            var UserToReturnDomainModel = new PeopleModel
            {
        ...
        public async Task<string> SoftDeletePeopleById(Guid Id)
        {
        ...
        public async Task<string> DeletePeopleById(Guid Id)
        {
